...
const DEFAULT_POSTER = `data:image/svg+xml;utf8,${encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="480" height="270"><rect width="100%" height="100%" fill="#212529"/><text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" fill="#fff" font-size="26" font-family="Arial">Cinema Poster</text></svg>')}`;

const seed = {
  config: { pointRate: 10000, starValue: 1000 },
  rooms: ['P1', 'P2', 'P3'],
  movies: [
    { id: 'm1', title: 'Hành Tinh Bóng Đêm', rating: 'T13', description: 'Khoa học viễn tưởng, hành động.', basePrice: 90000, poster: DEFAULT_POSTER },
    { id: 'm2', title: 'Mật Mã Cuối Cùng', rating: 'T16', description: 'Trinh thám nghẹt thở.', basePrice: 100000, poster: DEFAULT_POSTER },
  ],
  showtimes: [
...
function renderMovies() {
  ...
  db.movies.forEach((m) => {
    ...
    col.innerHTML = `<div class="card h-100"><div class="card-body">
      <img src="${m.poster || DEFAULT_POSTER}" class="movie-thumb mb-2" alt="${m.title}">
...
function renderAdmin() {
  ...
  byId('adminMovies').innerHTML = db.movies.map((m) => `<li class="list-group-item d-flex justify-content-between align-items-center"><span class="d-flex align-items-center gap-2"><img src="${m.poster || DEFAULT_POSTER}" class="movie-mini" alt="${m.title}">${m.title} (${m.rating})</span><span><button class="btn btn-sm btn-outline-primary" onclick="editMovie('${m.id}')">Sửa</button> <button class="btn btn-sm btn-outline-danger" onclick="deleteMovie('${m.id}')">Xóa</button></span></li>`).join('');
...
byId('movieForm').poster.addEventListener('change', async (e) => {
  const file = e.target.files?.[0];
  const preview = byId('moviePosterPreview');
  if (!file) {
  ...
  try {
    ...
    toast('Upload ảnh thất bại.', 'danger');
  }
});
...
byId('movieForm').addEventListener('submit', async (e) => {
  e.preventDefault();
  if (!ensureRole('admin')) return;
  const f = new FormData(e.target);
  const id = f.get('id').toString();
  const oldMovie = id ? movieById(id) : null;
  const file = byId('movieForm').poster.files?.[0];
  let poster = oldMovie?.poster || DEFAULT_POSTER;
  if (file) poster = await readFileAsDataURL(file);
  const payload = {
    id: id || uid('m'),
  ...
  save(db);
  e.target.reset();
  byId('moviePosterPreview').classList.add('d-none');
  byId('moviePosterPreview').src = '';
  toast('Lưu phim thành công.', 'success');
  render();
});

byId('createRoleAccountForm').addEventListener('submit', (e) => {
  e.preventDefault();
  if (!ensureRole('admin')) return;
  const f = new FormData(e.target);
  const email = f.get('email').toString().trim().toLowerCase();
...
window.editMovie = (id) => {
  ...
  const preview = byId('moviePosterPreview');
  preview.src = m.poster || DEFAULT_POSTER;
  preview.classList.remove('d-none');
};
